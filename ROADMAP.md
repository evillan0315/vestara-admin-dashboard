...
| **19** | Settings | Application Settings | ✅ Complete (~95%) |
...
| 19 | Application Settings | ✅ Complete | ~95% | **Backend:** Full CRUD routes with auth middleware, repository pattern, validation. **Multi-tenancy (2026-07-13):** Settings now scoped per-organization via compound unique `[organizationId, key]`. **Frontend:** SettingsPage with DataTable (key-value editor), SettingFormDialog for create/edit with JSON validation, TanStack Query hooks, toast notifications for success/error. |
