...
| **12–14** | User Management | User CRUD, Roles & Permissions, User Profile | ✅ Complete (100%) — refactored |
| **15–17** | Reusable UI | Forms, Data Table, Feedback Components | ✅ Complete (100%) — refactored |
...
| **23–24** | Security & Monitoring | Security Hardening, Monitoring | ⏳ Partial (~80%) |
| **25–26** | Testing & Performance | Testing, Performance Optimization | ⏳ In Progress (~35% testing / ~60% perf) |
| **27–30** | Production | Documentation, Deployment, CI/CD, Production Readiness | ✅ Complete (100%) — self-hosted live at vestara.meetlily.org |
| **—** | Marketing Assets | `apps/marketing-video` Remotion ad + HappyHorse-1.1-T2V cinematic prompt | ✅ Complete (100%) |
| **—** | AvatarUpload + Sidebar Logo | Reusable avatar component, sidebar app logo customization | ✅ Complete (100%) |
...
| 14 | User Profile | ✅ Complete | 100% | **Backend:** GET/PUT /profile (name, avatar), PUT /profile/password (current pw verification + bcrypt rehash), audit logging for updates. **Multi-tenancy (2026-07-13):** Profile scoped to user's organization. **Frontend:** ProfilePage with General tab + Security tab, OAuth detection, TanStack Query hooks. **New (2026-07-16):** Email change dialog, self-service account deletion, avatar file upload. Extended AuditAction enum. **Header & Profile Nav refresh (2026-07-16):** Shared `profileTabs` config, 6 profile tab routes, header `UserMenu` shortcuts, dark-luxury `theme.palette` header chrome. **PreferencesTab overhaul (2026-07-17):** Removed modal-only approach; added dedicated full-page tab with 16 persisted preference fields (theme mode, font family/size/weight, primary color, density, sidebar variant, border radius, contrast, notifications 4x, language, timezone, date format, privacy 3x). **Live theme preview + undo/reset:** Every discrete control applies immediately to ThemeContext; slider values debounced via `useDebounce`. Added `ThemePreviewCard`. Undo reverts to saved snapshot; reset-to-defaults restores factory values; save via `PUT /profile`. **UserMenu redesign:** Split into `UserMenuTrigger` + `UserMenuDropdown` with grouped sections. **Added `setFontWeight`** to `ThemeContextValue`. **Fix:** `prisma db push` added 9 columns to `user_profiles`; duplicate `/integrations` nav key resolved. All typechecks pass. AuditAction enum extended with `EMAIL_CHANGE` and `ACCOUNT_DELETION`. **Header & Profile Nav refresh (2026-07-16):** Extracted a shared `features/profile/tabs.tsx` config (`profileTabs` + `getProfileTabFromPath`) as the single source of truth for the 6 profile tabs (Overview, Security, Permissions, Activity, Preferences, Sessions) and their routes. `ProfilePage` now consumes this config for its tab bar + initial-tab derivation. The header `UserMenu` was refactored to render profile-tab shortcuts (Security/Permissions/Activity/Preferences/Sessions) wired to dedicated routes, with gold accents (`theme.palette.primary`) and full `theme.palette` usage (removed all hardcoded `colors` tokens). The entire header chrome — `Header`, `HeaderActions`, `HeaderIconButton`, `DateRangeButton`, `RefreshButton`, `ConnectionStatus`, `NotificationPopover`/`NotificationList`/`NotificationItem`, `MessagePopover`/`MessageList` — was migrated to `theme.palette` for consistent dark-luxury styling matching the Sidebar; the unused `HeaderSearch` component was removed. Routes for `/permissions`, `/activity`, `/preferences`, `/sessions` now mount `ProfilePage`. **Maintenance (2026-07-16):** Fixed profile tab routing — `ProfilePage` now syncs its active tab to `location.pathname` via a `useEffect` (previously the tab was locked to the first-render path), so navigating from the header `UserMenu` shortcuts to `/security`, `/activity`, etc. correctly switches the displayed tab. Typecheck, lint, and build all pass. **Enhancements (2026-07-XX):** Integrated avatar upload functionality with File Manager, allowing users to upload avatar images directly from the profile form using the existing file upload infrastructure. |
...
| 16 | Data Table | ✅ Complete | 100% | **Built:** Reusable DataTable component with theme-aware styling. Features: sortable column headers (toggling asc/desc/none), search + filter chips, pagination (page controls, page size selector), row selection (checkbox column), server-side support, loading skeleton, empty state (NoData/NoSearchResults/ErrorState), customizable column definitions (label, sortable, render, align, width). Used by UsersPage, SettingsPage, SystemLogsPage, and OrganizationsPage. **OrganizationsPage rewrite (2026-07-12):** Replaced raw MUI Table with DataTable component — client-side search/sort, Avatar + name column, slug, member count, created date, edit action. Matches UsersPage layout pattern. **Refactoring (2026-07-XX):** OrganizationsPage split from 420 lines into 4 focused files — `useOrganizationsPage.ts` hook, `OrganizationsDialog`, `OrganizationsTableColumns`. Stale `UsersFilterChips.tsx` removed. |
...
| 18 | File Manager | ✅ Complete | 100% | **Backend:** Storage provider abstraction (Local, Cloudinary, S3, Google Drive) with factory pattern. File model with org-scoped CRUD, folder hierarchy, metadata. **API:** `/api/v1/files` — list (paginated/filtered), folder contents, upload (multi-file, 100MB), create folder, rename/move, signed download URLs, bulk move/delete. **Storage:** Org-level config via SystemSetting `storage` key — supports LOCAL (default), CLOUDINARY (with provided credentials: API 914425692112145), S3, GOOGLE_DRIVE. **Audit:** All operations logged. **Frontend (2026-07-12):** FileManagerPage with DataTable (sortable columns, row selection, search), folder cards with click-to-navigate, clickable MUI breadcrumbs, upload dialog with drag-and-drop zone and XHR progress bar (LinearProgress), create folder dialog, rename dialog, move dialog (select target folder), delete + bulk delete with ConfirmDialog, image preview dialog, download via URL, list/grid view toggle. **API client:** `apps/web/src/api/files.ts` with XHR upload progress. **Hooks:** `apps/web/src/features/files/hooks.ts` — TanStack Query hooks for folder contents, upload, create folder, rename, move, delete, bulk delete, stats. **Route:** `/files` in SYSTEM nav group. Build + typecheck + lint all pass. **Enhancements (2026-07-XX):** Integrated avatar upload functionality used by UserProfile and UserFormDialog components, providing a reusable image upload interface with preview capabilities. **Refactoring (2026-07-XX):** FileManagerPage split from 1,231 lines into 7 focused files (<600 lines each) — `useFileManagerPage.ts` hook, `FileManagerBreadcrumbs`, `FileManagerToolbar`, `FileManagerFolderGrid`, `FileManagerTableColumns`, `FileManagerDialogs` (7 dialogs). |
...
| 20 | Reports | ✅ Complete | 100% | **Backend:** Full report generation API with async job processing (`/api/v1/reports`). Supports 4 report types: audit-logs, users, activity, system-logs. 3 export formats: CSV, Excel (xlsx), PDF. Uses `exceljs` for Excel, `pdfkit` for PDF. Org-scoped data access with filters (date range, action, entity, user). Files stored as base64 data URLs (production would use S3/Blob). **Frontend:** ReportsPage at `/reports` with DataTable showing report history, generate dialog with type/format/date filters, download button, delete action. Export hook for DataTable components. Added to navigation sidebar under "OPERATIONS" group. TanStack Query hooks for all operations. **Refactoring (2026-07-XX):** ReportsPage split from 626 lines into 5 focused files — `useReportsPage.ts` hook, `GenerateReportDialog`, `ReportsTableColumns`, `ReportsStatsCards`. **Reports Enhancement (2026-07-28):** Three tiers of reports page enhancements — **Tier 1:** MUI-ified Generate dialog with date preset chips (Today/This Week/This Month/Last Month/Last 90 Days/Custom), column selection picker, schedule (cron) and email delivery fields; WebSocket real-time progress events (`WS_EVENT.REPORT_STATUS`) replacing polling; search/sort support in list endpoint; report stats endpoint. **Tier 2:** Report Templates CRUD (new `ReportTemplate` Prisma model, backend endpoints, frontend panel with card grid); inline data preview endpoint (first 10 rows); report comparison dialog (select 2-5 reports, side-by-side attribute table). **Tier 3:** Reports dashboard widget integrated into DashboardPage; PDF branding (org logo embedding); row selection with bulk delete; export presets (selected columns remembered per type). Added `ReportTemplate` to schema with org-scoped cascade. All typecheck + lint pass. |
...
| 23 | Security | ✅ Complete | 100% | Security headers (Helmet CSP/HSTS/Referrer-Policy/Permissions-Policy), CORS, JWT auth + refresh rotation, Zod validation, audit trail, **rate limiting** (auth 5/15m, api 100/min, health permissive), **CSRF/origin verification** on state-changing requests, **password policy** (length + character classes + common-password blocklist + `POST /auth/password-strength`), **XSS protection** (CSP + sanitize middleware stripping script/event-handlers/`javascript:` URIs + `isSafeQueryValue` NoSQL/prototype-pollution guard). BigInt serialization polyfill. **Account lockout (2026-07-17):** 5 failed logins → 30 min lockout (`failedLoginAttempts` + `lockedUntil` on User schema). Counter resets on successful login; lockout check prevents timing enumeration. `ACCOUNT_LOCKED` error code. **X-Request-Id (2026-07-17):** UUID header on every request for distributed tracing, attached to logs. **Body size limits:** Global JSON parser reduced from 10 MB → 1 MB. **WebSocket rate limiting:** 10 connection attempts per IP per minute (sliding window). **CSP tightening:** `img-src` restricted to `self+data+blob` in production; `frame-ancestors: 'none'` via CSP (replaced Helmet `frameguard`). |
| 24 | Monitoring | ⏳ Partial | ~70% | Audit trail service + request logging exist. Audit logs API routes implemented (GET /audit-logs paginated+filterable, GET /audit-logs/:id). **Frontend:** SystemLogsPage built with DataTable, action/entity/date filters, action chips with color-coded icons, formatted timestamps. **Error tracking (2026-07-12):** Global error handler now persists API request errors as audit-log entries (`AuditAction.ERROR`, `entity: 'api'`) for authenticated requests — metadata captures status, code, message, method, path, and stack for 5xx. System Logs page styles the `error` action (red chip + Error icon); dashboard action label added. **Multi-tenancy (2026-07-13):** Audit logs scoped to organization. **Refactoring (2026-07-XX):** SystemLogsPage split into `useSystemLogsPage.ts` hook and `SystemLogsColumns.tsx` column factory. **Still missing:** performance metrics |
...
| 28 | Deployment | ✅ Complete | 100% | **Self-hosted live at vestara.meetlily.org** — Ubuntu 22.04 VPS with Nginx (SPA + API proxy + WebSocket + Let's Encrypt TLS), PM2 (forked mode, `.env.deploy` auto-load, systemd auto-start), PostgreSQL 17, Redis 8. `scripts/deploy.sh` for SSH deploys (atomic symlink swap, `--api` flag for server-side rebuild with tsbuildinfo cleanup + ownership fix). **Vercel alternative:** API at `vestara-admin-api.vercel.app`, Web at `vestara-admin-web.vercel.app`. **Trust proxy fix:** Express `trust proxy` set for Nginx reverse proxy so rate limiting identifies real client IPs. **Settings route fix:** `GET /settings/:key` returns `null` gracefully for missing optional settings (e.g. `app_logo_url`). |
...
| 30 | Production Readiness | ⏳ Partial | ~75% | Both apps deployed and serving (API health 200, Web login 200). CORS verified. OAuth callbacks functional. GitHub Actions CI/CD configured for API. **Multi-tenancy foundation complete** — Organization model, org-scoped resources, org-level RBAC. **Self-hosted live at vestara.meetlily.org (2026-07-17):** Ubuntu 22.04 VPS, Nginx + Let's Encrypt TLS, PM2 with systemd auto-start, PostgreSQL 17, Redis 8. Trust proxy fix for rate limiting behind Nginx. Settings route returns `null` for missing optional keys. Database seeded (2 orgs × 3 users). **Local dev (2026-07-16):** `docker-compose.yml` (Postgres 17 + Redis 8), Vite dev proxy `/api` → `:5000`, git-ignored localhost `.env`, and `pnpm dev:local` one-command bootstrap (`scripts/dev-local.sh`). **Config fix (2026-07-16):** standardized S3 credential env vars to AWS-conventional `S3_ACCESS_KEY_ID` / `S3_SECRET_ACCESS_KEY`. **Live preview + undo/reset (2026-07-17):** PreferencesTab with 16 fields, real-time ThemeContext sync, `ThemePreviewCard`, debounced sliders, undo/reset/save controls. UserMenu redesign with `UserMenuTrigger`/`UserMenuDropdown`. Database migration pushed (`prisma db push`). All typechecks pass. **Missing:** final security audit, performance benchmarks |
...
| — | AvatarUpload Component + Sidebar Logo | ✅ Complete | 100% | **AvatarUpload (2026-07-28):** Reusable `AvatarUpload` component with view-only + editable modes, camera overlay on hover, file picker, loading spinner, initials fallback, custom size presets (small/medium/large/px), and SVG support. Integrated into **7 sites**: ProfilePage (user avatar upload), UserFormDialog (user avatar in edit), OrganizationsDialog (org logo upload), UsersTableColumns, OrganizationsTableColumns, UserMenuTrigger, UserMenuDropdown, Header. Replaced the raw Avatar + hidden file input + camera button pattern throughout. **Sidebar Logo Customization:** New `useAppLogo` hook reads/writes `app_logo_url` SystemSetting. `Logo` component accepts optional `src` prop. Sidebar shows a camera overlay for `super_admin`/`admin` users to upload/replace the logo in-place. All typechecks + build + lint pass. |
