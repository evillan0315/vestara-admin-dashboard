...
import { z } from 'zod';
import { UserRole, AuditAction, EntityType } from '@vestara/types';
import {
  createUserSchema,
  updateUserSchema,
...
registry.register(
  ...
  z.object({ email: z.string().email() }).openapi('ForgotPasswordRequest'),
);
registry.register(
  'ResetPasswordRequest',
  z.object({ token: z.string(), password: z.string().min(8) }).openapi('ResetPasswordRequest'),
);
