...
- Theme: extend `apps/web/src/styles/theme.ts` / `apps/web/src/theme/tokens.ts` — do **not** hardcode colors; read from `theme` or `tokens`. All header chrome (`Header`, `UserMenu`, popovers, status indicators) and the profile page use `theme.palette` (gold = `primary.main`, text = `text.primary/secondary/disabled`, borders = `divider`, surfaces = `background.paper`) for a consistent dark-luxury look. The 6 profile tabs and their routes are defined once in `apps/web/src/features/profile/tabs.tsx` (`profileTabs` + `getProfileTabFromPath`) and shared by `ProfilePage` and the header `UserMenu`. `ProfilePage` syncs its active tab to `location.pathname` so header shortcuts route to the correct tab.
- **Header title**: the page title/subtitle shown in the `Header` is derived automatically from the current route via `apps/web/src/layouts/routeTitles.ts` (`getRouteTitle(pathname)`), which combines the sidebar `navConfig` (excluding roadmap "soon" placeholders) with the profile tab routes. Pages can still override it via the `Header` `title`/`subtitle` props.
- **Global Search (Cmd/Ctrl+K)**: `apps/web/src/components/layout/GlobalSearchDialog.tsx` is data-driven — navigation results are generated from `navConfig`, live user search queries `usersApi.list({ search })` (debounced) and deep-links to `/users?focus=<id>`, and an **All / This Page** scope toggle limits results to the active route. The header no longer shows the online-users `PresenceIndicator` (only the real-time `ConnectionStatus` dot remains).
- **Date range**: dashboard/analytics date filtering uses the shared `apps/web/src/features/calendar/` module (`DateRangeContext` + `CalendarDatePicker` + `CalendarPopover`), wired through `AppProvider`.
